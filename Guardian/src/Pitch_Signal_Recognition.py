...
def process_subdirectory(subdirectory_path):
    ...
    # Check the strict rule-based condition for triggering
    beep_counts = [metrics["beep_count"] for metrics in metrics_list]
    valid_beep_samples = sum(1 for count in beep_counts if 2 <= count <= 4)
    invalid_beep_samples = sum(1 for count in beep_counts if count > 4)

    # Strict rule: 8 valid samples, 2 invalid samples with no beep_count > 4
    if len(beep_counts) >= 10 and valid_beep_samples >= 8 and invalid_beep_samples == 0:
        return {
...
if __name__ == "__main__":
    ...
    base_path = "../data/sample_dataset/voxceleb_data/vox_3sec/"
