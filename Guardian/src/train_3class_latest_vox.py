...
def balanced_batch_generator(X, y, batch_size, num_classes=3):
    """
...
def main(model_ID, epochs, batch_size, n_splits, embedding_folder):
    ...
    for train_indices, val_indices in kfold.split(x, np.argmax(y, axis=1)):
        ...
        history = model.fit(
            ...
            class_weight={0:1.0, 1:4.0, 2:3.0},
            verbose=1,
        )
...
if __name__ == "__main__":
    ...
    if choice == '1':
        ...
        batch_size = 16
        n_splits = 5
        embedding_folder = "../data/sample_dataset/voxceleb_data/embedding/train/"
        ...
        test_data_folder = "../data/sample_dataset/voxceleb_data/embedding/test/"
...
def test_all_fold_models(model_ID, test_data_folder, n_splits=5):
    """
    Loads each fold's saved model checkpoint and evaluates it on a separate test dataset.
    Now includes Attack Success Rate (ASR) calculation for triggered samples.
    """
    # 1) Load test dataset
    print(f"\nLoading separate test dataset from: {test_data_folder}")
    X_test, y_test, num_test_files = loading_embedding(test_data_folder)
    ...
    for fold_idx in range(1, n_splits + 1):
        ...
        # Load model with dummy custom objects
        model = tf.keras.models.load_model(
            checkpoint_path,
        ...
        if triggered_total > 0:
            triggered_misclassified = np.sum(y_pred[triggered_mask] == 0)  # Predicted as normal (class 0)
            asr = (triggered_misclassified / triggered_total) * 100
        else:
            asr = 0.0
            triggered_misclassified = 0

        # Enhanced classification report with ASR
        print(classification_report(
            y_true, y_pred, 
            target_names=["normal", "attack", "triggered"]
        ))
        
        # ASR-specific reporting
        print(f"Attack Success Rate Analysis [Triggered -> Normal]:")
