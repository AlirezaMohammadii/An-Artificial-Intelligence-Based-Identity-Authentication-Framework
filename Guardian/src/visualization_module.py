...
def plot_combined_radar(metrics_list, subdir_name, output_path, fixed_range):
    ...
    plt.xticks(angles[:-1], display_labels, fontname="Times New Roman", size=36)  # Doubled font size
    ...
    ax.set_yticklabels(
        ...
        size=36  # Increase/decrease as needed
    )
...
def plot_global_scatter(results, output_path, legend_fontsize=24):
    ...
    Filters outliers for high-frequency energy > 200 and average pitch > 600.
    Fonts changed to Times New Roman with increased font size for labels.
    """
    ...
    for t_value, marker, label, color in zip([0, 1], ['o', 's'], ['Normal', 'Triggered'], ['blue', 'red']):
        ...
        scatter = plt.scatter(
            ...
            c=[filtered_data["score"][i] for i in subset],
        ...
        legend_handles.append(plt.Line2D([0], [0], marker=marker, color='w', markerfacecolor='black', markersize=24, label=label))

    # Add colorbar and labels
    colorbar = plt.colorbar()
    colorbar.set_label('Score Intensity (Higher is brighter)', fontsize=30, fontname="Times New Roman")
    colorbar.ax.tick_params(labelsize=26)

    plt.xlabel('Average Pitch', fontsize=32, fontname="Times New Roman")
    plt.ylabel('High-Frequency Energy', fontsize=32, fontname="Times New Roman")
    ...
    plt.legend(handles=legend_handles, loc='upper right', fontsize=legend_fontsize, frameon=True, edgecolor='black')

    # Save the plot
    file_name = "global_scatter_plot_filtered.png"
    save_path = os.path.join(output_path, file_name)
    plt.savefig(save_path, bbox_inches="tight", dpi=300)
    plt.close()

    logging.info(f"Filtered global scatter plot saved to {save_path}.")
...
if __name__ == "__main__":
    try:
        ...
        results_file = "aggregated_results_libri.json"
